...
async function constantDeclarationsFromFileContent(fileContent)  {
  let result = {};
  const constantRegex = /^(?:export\s+)?\s*(?:const|let|var)\s+(?<constantName>[A-Z][A-Z0-9_]*)\s*=\s*(?<constantValueStart>[\[{("'])/g;
  ...
  for (const constantDeclarationMatch of constantDeclarations) {
    if (Number.isInteger(constantDeclarationMatch?.index) && constantDeclarationMatch.groups?.constantValueStart) {
      ...
      const endIndex = constantPlusRemainingContent.indexOf(endCharacter)
...
function codeBlockFromConstantBlocks(codeBlockString, constantBlocks, functionTranslations, importUrl) {
  for (let [ constantName, constantContent ] of Object.entries(constantBlocks)) {
    const definition = constantContent.split("\n")[0];
    const urlSegments = importUrl.split("/");
    const newConstantName = `_inlined_${ urlSegments[urlSegments.length - 1].replace(/[^\w]/g, "_") }_${ constantName }`;
    functionTranslations.push({ constantName, newConstantName, importUrl });
    const newDefinition = `${ newConstantName } = `;

    let newConstantBlock = constantContent.replace(definition, newDefinition).split("\n").map(line => `  ${ line }`).join("\n");
    newConstantBlock = `\n  ${ newConstantBlock.trim() }${ newConstantBlock.trim().endsWith(",") ? "" : "," }\n`;

    const endBracket = codeBlockString.lastIndexOf("}");
    codeBlockString = codeBlockString.substring(0, endBracket) + newConstantBlock + codeBlockString.substring(endBracket);
  }
