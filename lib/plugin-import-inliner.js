 ...
 */
export async function inlineImportsFromGithub(entryPoint, codeBlockString, functionTranslations, constantTranslations) {
  ...
  for (const importUrl of importUrls) {
    ...
    if (functionBlocks) {
      codeBlockString = codeBlockFromFunctionBlocks(codeBlockString, functionBlocks, functionTranslations, importUrl);
    }
    ...
    const constantBlocks = await constantDeclarationsFromFileContent(importFileContent);
    if (constantBlocks) {
      codeBlockString = codeBlockFromConstantBlocks(codeBlockString, constantBlocks, constantTranslations, importUrl);
    }

    constantTranslations.forEach(translation => {
      const replaceConstantRegex = new RegExp(`(?<![\\_\\.])\\b${ translation.constantName }\\b`, "g");
      codeBlockString = codeBlockString.replace(replaceConstantRegex, `this.${ translation.newConstantName }`);
    });

    // Recurse to check if entryPoint.content has imports of its own to inline
    codeBlockString = await inlineImportsFromGithub({ url: importUrl, content: importFileContent }, codeBlockString,
      functionTranslations, constantTranslations);
  }
...
//----------------------------------------------------------------------
async function constantDeclarationsFromFileContent(fileContent)  {
  let result = {};
  const constantRegex = /^(?:export\s+)?\s*(?:const|let|var)\s+(?<constantName>[A-Z][A-Z0-9_]*)\s*=\s*(?<constantValueStart>[\[{("'])/gm;
  ...
  for (const constantDeclarationMatch of constantDeclarations) {
    if (Number.isInteger(constantDeclarationMatch?.index) && constantDeclarationMatch.groups?.constantValueStart) {
      ...
      const constantPlusRemainingContent = fileContent.substring(constantStartIndex);
      const endCharacter = { "(": ")", "{": "}", '"': '"', "'": "'", "[": "]" }[constantDeclarationMatch.groups?.constantValueStart];
      const startIndex = constantPlusRemainingContent.indexOf(constantDeclarationMatch.groups?.constantValueStart);
      const endOffset = constantPlusRemainingContent.substring(startIndex + 1).indexOf(endCharacter);
      const endIndex = startIndex + endOffset + 1;

      if (endIndex !== -1 && startIndex !== -1) {
        const constantContent = constantPlusRemainingContent.substring(startIndex, endIndex + 1);
        const constantName = constantDeclarationMatch.groups?.constantName;
        console.log("Got constant block length", constantContent?.length, "for", constantName);
        result[constantName] = constantContent;
      }
    }
  }
...
//----------------------------------------------------------------------
function importUrlsFromContent(content, extension, contentFileUrl) {
  let match;
  const importUrls = [];
...
function codeBlockFromFunctionBlocks(codeBlockString, functionBlocks, functionTranslations, importUrl) {
...
//----------------------------------------------------------------------
// Translate constantBlock entries into new constant definitions and insert them into the codeBlockString
// @param {string} importUrl - URL of the file whose exported objects will be captured
function codeBlockFromConstantBlocks(codeBlockString, constantBlocks, constantTranslations, importUrl) {
  const result = codeBlockString;
  for (let [ constantName, constantContent ] of Object.entries(constantBlocks)) {
    const urlSegments = importUrl.split("/");
    const importNameSegment = urlSegments[urlSegments.length - 1].replace(/[^\w]/g, "_");
    const newConstantName = `_inlined_${ importNameSegment }_${ constantName }`;
    constantTranslations.push({ constantName, newConstantName, importUrl });

    const endIndex = result.lastIndexOf("}");
    const contentLines = constantContent.split("\n");
    const formattedContent = [ contentLines[0] ].concat(contentLines.slice(1).map(line => `  ${ line }`)).join("\n");
    const newDeclaration = `\n  ${ newConstantName }: ${ formattedContent },\n`;
    const contentBefore = codeBlockString.substring(0, endIndex);
    const contentAfter = codeBlockString.substring(endIndex);

    // Insert the new constant immediately before the end bracket of the object
    codeBlockString = `${ contentBefore }${ newDeclaration }${ contentAfter }`;
  }
