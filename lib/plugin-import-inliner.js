...
export const fileContentFromUrl = async (url) => {
  ...
  if (moduleFetchResponse?.ok && (fileContent = await moduleFetchResponse.text())) {
    // Originally (circa 2023) we called the file like "https%3A%2F%2Fgithub.com%2Falloy-org%2Fai-plugin%2Fblob%2Fmain%2Fbuild%2Fcompiled.js". But as of 2024, GH returns incomprehensible torrent of characters, so plan B is to just request plain file text
    // The hope is that GH won't return cached file text to us, as it did during one round of early testing
