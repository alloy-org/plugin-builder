...
export async function inlineImportsFromGithub(entryPoint, codeBlockString, functionTranslations, constantTranslations) {
  ...
  await constantDeclarationsFromFileContent(content, constantTranslations);
  ...
  for (const importUrl of importUrls) {
    ...
    if (importFileContent) {
      console.info("Received", importFileContent.length, "bytes from", importUrl, ". Now processing constants & functions...");
    } else {
    ...
    await constantDeclarationsFromFileContent(importFileContent, constantTranslations);
...
//----------------------------------------------------------------------
async function constantDeclarationsFromFileContent(fileContent, constantTranslations)  {
