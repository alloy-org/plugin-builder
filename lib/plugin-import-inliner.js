...
export async function inlineImportsFromGithub(entryPoint, codeBlockString, functionTranslations) {
  ...
  for (const importUrl of importUrls) {
    ...
    if (constantBlocks) {
      codeBlockString = codeBlockFromConstantBlocks(codeBlockString, constantBlocks, functionTranslations, importUrl);
    }
...
// @param {string} importUrl - URL of the file whose exported objects will be captured
function codeBlockFromConstantBlocks(codeBlockString, constantBlocks, functionTranslations, importUrl) {
  ...
  for (let [ constantName, constantContent ] of Object.entries(constantBlocks)) {
    ...
    functionTranslations.push({ constantName, newConstantName, importUrl });

    const startIndex = codeBlockString.indexOf(constantName);
    const endIndex = result.lastIndexOf("}");
    const line = `${ newConstantName }: ${ constantContent },\n`;
    const contentBefore = codeBlockString.substring(0, startIndex);
    const contentAfter = codeBlockString.substring(endIndex);
    codeBlockString = `${ contentBefore }${ line }${ contentAfter }`;
    codeBlockString = codeBlockString.replaceAll(constantName, newConstantName);
  }
