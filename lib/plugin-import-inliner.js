...
export async function inlineImportsFromGithub(entryPoint, codeBlockString, functionTranslations, constantTranslations) {
  ...
  const constantBlocks = await constantDeclarationsFromFileContent(entryPoint.content);
  if (constantBlocks) {
    codeBlockFromConstantBlocks(codeBlockString, constantBlocks, constantTranslations);
  }
  ...
  for (const importUrl of importUrls) {
    ...
    codeBlockString = await inlineImportsFromGithub(newEntryPoint, codeBlockString, functionTranslations, constantTranslations);
  }
...
// @param {string} importUrl - URL of the file whose exported objects will be captured
function codeBlockFromConstantBlocks(codeBlockString, constantBlocks, constantTranslations) {
