...
const plugin = {
  ...
  async _syncUrlToNote(app, repoUrl) {
    ...
    if (entryPoint.url) {
      ...
      const mainPluginBlock = entryPoint.content.match(/=[\s]*(\{\n[\S\s]*\n\})/)?.at(1);
      const constantTranslations = [];
      const functionTranslations = [];
      let newPluginBlock = await inlineImportsFromGithub(this, entryPoint, mainPluginBlock, functionTranslations, constantTranslations);
      if (newPluginBlock) {
