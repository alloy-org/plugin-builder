...
describe("This here plugin", () => {
  ...
  it("should allow specifying constants", async () => {
    ...
    await plugin.noteOption["Refresh"].run(app);
    const noteContent = note.content();
    expect(noteContent).toContain(`_inlined_plugin_constants_js_DEFAULT_BRANCH: "main",`);
    expect(noteContent).toContain(`_inlined_plugin_constants_js_CODE_HEADING: "Code block",`);
    const entryLocations = ` _inlined_plugin_constants_js_ENTRY_LOCATIONS: [
      "lib/plugin.js",
      "plugin.js",
      "index.js"
    ],`;
    entryLocations.split("\n").forEach((line) => {
      expect(noteContent).toContain(line.trim());
    });
    expect(noteContent).toContain(`const heading = this._inlined_plugin_constants_js_CODE_HEADING;`);
  });
