...
describe("This here plugin", () => {
  ...
  it("should allow specifying constants", async () => {
    const content = `Entry: ${ REPO_URL }/src/constants-test.js
    # Code block`.replace(/^[\s]*/gm, "");
    ...
    await plugin.noteOption["Refresh"].run(app);
    const noteContent = note.content();
    expect(noteContent).toContain(`_inlined_DEFAULT_BRANCH = "main"`);
    expect(noteContent).toContain(`_inlined_CODE_HEADING = "Code block"`);
    expect(noteContent).toContain(`_inlined_ENTRY_LOCATIONS = {`);
    expect(noteContent).toContain(`"lib/plugin.js",`);
  });
